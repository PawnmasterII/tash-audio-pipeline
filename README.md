...
| `stage2_cueword.py` | **Vosk** offline ASR + energy/VAD gate; no key, spots distress and reassurance words | `CueWordEvent` + `speech_present` |
...
6. **Configurable word sets via `cue_words` param.** `CueWordDetector` defaults
   to `config.CUE_WORDS = ["help"]`; pass `cue_words=config.CUE_WORDS +
   config.REASSURANCE_WORDS` to also recognise "fine"/"okay"/"ok". The emitted
   `CueWordEvent.category` field (`"distress"` or `"reassurance"`) lets consumers
   branch without re-importing the word lists.
